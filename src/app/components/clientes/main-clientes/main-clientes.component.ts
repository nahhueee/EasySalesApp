...
import { ClientesAddmodComponent } from '../clientes-addmod/clientes-addmod.component';
...
export class ClientesComponent implements OnInit {
  ...
  //Selecciona todas las filas si no están todas seleccionadas; en caso contrario, borra la selección.
    ...
    Buscar(event?: PageEvent){
      ...
      this.clientesService.ObtenerTotalClientes(filtro)
        .then(res => {
          this.paginator.length = res;

          // Busca los clientes en base a la pagina seleccionada
          this.clientesService.ObtenerClientes(filtro)
          .then(response => {

            //Llenamos la tabla con los resultados
            this.clientes = [];
            for (let i = 0; i < response.length; i++) {
              this.clientes.push(new Cliente(response[i]));
            }
  ...
  //#region MODAL/ABM
    ...
    Agregar(){
      ...
      this.dialog.open(ClientesAddmodComponent, this.dialogConfig)
                  .afterClosed()
      ...
      if(row==null){ //Si no hizo doble click sobre una celda y selecciono mas de una
        ...
        if(nroSeleccionados>0){
          for (let i = 0; i < nroSeleccionados; i++) {
            ...
            this.dialogConfig.width = "400px";
            this.dialogConfig.data = {cliente:this.seleccionados.selected[i].id} //Pasa como dato el id de cliente
            this.dialog.open(ClientesAddmodComponent, this.dialogConfig)
                    .afterClosed()
                    .subscribe((actualizar:boolean) => {
                      if (actualizar){
        ...
        this.dialogConfig.width = "400px";
            this.dialogConfig.data = {cliente:row.id} //Pasa como dato el id de cliente
            this.dialog.open(ClientesAddmodComponent, this.dialogConfig)
                    .afterClosed()
                    .subscribe((actualizar:boolean) => {
                      if (actualizar){
                  ...
                  .subscribe(async (confirmado: Boolean) => {
                    ...
                    if (confirmado) {
                      ...
                      for (let i = 0; i < nroSeleccionados; i++) {
                        ...
                        await this.clientesService.Eliminar(this.seleccionados.selected[i].id)
                        .then(response => {
